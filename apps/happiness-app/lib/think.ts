...
import { orchestrateAgents, AgentType } from './langsmith';
...
Respond with JSON: { "summary": "your natural response here" }`
  ...
  try {
    ...
    // Safety check: Assess risk level of AI response
    const riskAssessment = assessRisk(responseText);
    if (riskAssessment.level === 'high') {
      Logger.warn('High-risk AI response detected', {
      ...
      return {
        summary:
          "I'm here for you. If you're going through something difficult, please reach out to someone you trust or a professional. You don't have to go through this alone.",
        error: 'High-risk content filtered',
      };
    }

    Logger.info('AI response received', {
      length: responseText.length,
      hasNextStep: !!(content.next_step || content.nextStep),
    });
